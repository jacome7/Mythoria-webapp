...
export default function RevolutPayment({
  ...
  useEffect(() => {
    ...
    const loadAndInitialize = async () => {
      try {
        ...
        if (isMounted) {
          setError(error instanceof Error ? error.message : tRevolutPayment('errors.initializationFailed'));
          setIsLoading(false);
        }
      }
    };
  ...
  }, [orderToken, orderAmount, disabled, onPaymentSuccess, onPaymentError, onPaymentCancel, tRevolutPayment]);
