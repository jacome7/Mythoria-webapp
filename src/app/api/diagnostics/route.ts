  ...
  try {
    ...
    for (const locale of locales) {
      try {
        ...
        diagnostics.errors.push(`Failed to process locale ${locale}: ${e instanceof Error ? e.message : String(e)}`);
      }
    }
  } catch (e) {
    diagnostics.errors.push(`Failed to read messages directory: ${e instanceof Error ? e.message : String(e)}`);
  }
