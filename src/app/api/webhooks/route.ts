...
async function ensureInitialCredits(authorId: string) {
  ...
  const existingInitial = await db
    ...
    .from(creditLedger)
    .where(
      and(eq(creditLedger.authorId, authorId), eq(creditLedger.creditEventType, 'initialCredit')),
    )
    .limit(1);
  ...
  try {
    ...
    const extractPreferredName = (e: WebhookEvent): string | undefined => {
      ...
      const privateMetadata = (
        data as { private_metadata?: { profile?: { preferredName?: unknown } } }
      ).private_metadata;
    ...
    if ((updateResult.rowCount || 0) === 0) {
      console.log(
        'No user found with clerkUserId:',
        id,
        'trying to update by email:',
        primaryEmail.email_address,
      );
...
async function handleUserDeleted(evt: WebhookEvent) {
  ...
  try {
    ...
    const deleteResult = await db.delete(authors).where(eq(authors.clerkUserId, id));
