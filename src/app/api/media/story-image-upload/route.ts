...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!author) return NextResponse.json({ success: false, error: 'Unauthorized' }, { status: 401 });
    ...
    if (!story || story.authorId !== author.authorId) {
      return NextResponse.json({ success: false, error: 'Story not found or access denied' }, { status: 404 });
    }

    const resp = await sgwFetch('/ai/media/story-image-upload', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ storyId, imageType, chapterNumber, contentType, dataUrl, currentImageUrl })
    });
