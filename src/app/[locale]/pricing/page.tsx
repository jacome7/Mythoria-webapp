				...
				<section id="service-costs" className="my-16">
					{loading ? (
						...
						<div className="flex justify-between items-center mb-4">
							...
							<p className="font-semibold text-right">{tPricingPage('serviceCosts.creditsHeader')}</p>
						</div>
						...
						<div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
							{creditPackages.map((pkg: CreditPackage) => (
								<div key={pkg.id} className={`card bg-base-200 shadow-xl relative ${pkg.bestValue ? 'border-2 border-accent' : pkg.popular ? 'border-2 border-secondary' : ''}`}>
									{pkg.bestValue && <div className="badge badge-accent absolute -top-3 -right-3 p-2">{tPricingPage('creditPackages.badges.bestValue')}</div>}
									{pkg.popular && <div className="badge badge-secondary absolute -top-3 -right-3 p-2">{tPricingPage('creditPackages.badges.popular')}</div>}
									<div className="card-body items-center text-center">
										<div className="text-4xl text-primary mb-2">{getIconComponent(pkg.icon)}</div>
										<h3 className="card-title text-3xl">{pkg.credits} {tPricingPage('creditPackages.credits')}</h3>
					...
					<div className="hero-content flex-col lg:flex-row max-w-none w-full px-0 sm:px-2">
						...
						<div>
							...
							<ul className="list-disc list-inside space-y-2 text-gray-700">
								<li><strong>{tPricingPage('whyCredits.benefits.flexibility.title')}</strong> {tPricingPage('whyCredits.benefits.flexibility.description')}</li>
								<li><strong>{tPricingPage('whyCredits.benefits.savings.title')}</strong> {tPricingPage('whyCredits.benefits.savings.description')}</li>
								<li><strong>{tPricingPage('whyCredits.benefits.gifting.title')}</strong> {tPricingPage('whyCredits.benefits.gifting.description')}</li>
								<li><strong>{tPricingPage('whyCredits.benefits.simplicity.title')}</strong> {tPricingPage('whyCredits.benefits.simplicity.description')}</li>
							</ul>
						</div>
					</div>
				</section>
					...
					<div className="space-y-4 max-w-4xl mx-auto px-0 sm:px-4">
						<div tabIndex={0} className="collapse collapse-plus border border-base-300 bg-base-200 rounded-box">
							<div className="collapse-title text-sm sm:text-l font-medium flex items-center">
								<FaQuestionCircle className="mr-2 text-primary" /> {tPricingPage('faq.questions.expiration.question')}
							</div>
						...
						<div tabIndex={1} className="collapse collapse-plus border border-base-300 bg-base-200 rounded-box">
							<div className="collapse-title text-sm sm:text-l font-medium flex items-center">
								<FaQuestionCircle className="mr-2 text-primary" /> {tPricingPage('faq.questions.refund.question')}
							</div>
							<div className="collapse-content">
						...
						<div tabIndex={2} className="collapse collapse-plus border border-base-300 bg-base-200 rounded-box">
							<div className="collapse-title text-sm sm:text-l font-medium flex items-center">
								<FaQuestionCircle className="mr-2 text-primary" /> {tPricingPage('faq.questions.payment.question')}
							</div>
							<div className="collapse-content">
						...
						<div tabIndex={3} className="collapse collapse-plus border border-base-300 bg-base-200 rounded-box">
							<div className="collapse-title text-sm sm:text-l font-medium flex items-center">
								<FaQuestionCircle className="mr-2 text-primary" /> {tPricingPage('faq.questions.shipping.question')}
							</div>
							<div className="collapse-content">
				...
				<div className="modal modal-open">
					<div className="modal-box w-10/12 max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg mx-4">
						...
						<div className="modal-action">
                                                        <button className="btn" onClick={closeModal}>{tPricingPage('infoModal.close')}</button>
						</div>
					</div>
