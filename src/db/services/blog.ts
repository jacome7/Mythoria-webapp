...
export const blogService = {
  async createPost(input: CreatePostInput) {
    const [existing] = await db.select({ id: blogPosts.id }).from(blogPosts).where(eq(blogPosts.slugBase, input.slugBase));
  ...
  async upsertTranslation(input: UpsertTranslationInput) {
    const [existing] = await db.select({ id: blogPostTranslations.id }).from(blogPostTranslations)
      .where(and(eq(blogPostTranslations.postId, input.postId), eq(blogPostTranslations.locale, input.locale)));
    if (existing) {
      const [updated] = await db.update(blogPostTranslations).set({
        slug: input.slug,
        title: input.title,
        summary: input.summary,
        contentMdx: input.contentMdx,
        updatedAt: new Date(),
      }).where(eq(blogPostTranslations.id, existing.id)).returning();
      return updated;
    } else {
      const [created] = await db.insert(blogPostTranslations).values({
        postId: input.postId,
        locale: input.locale,
        slug: input.slug,
        title: input.title,
  ...
  async publish(postId: string) {
    const [updated] = await db.update(blogPosts).set({ status: 'published', publishedAt: new Date(), updatedAt: new Date() }).where(eq(blogPosts.id, postId)).returning();
    return updated;
  },

  async archive(postId: string) {
    const [updated] = await db.update(blogPosts).set({ status: 'archived', updatedAt: new Date() }).where(eq(blogPosts.id, postId)).returning();
    return updated;
  },
  ...
  async getPublishedBySlug(locale: BlogLocale, slug: string) {
    const [row] = await db.select({
      ...
      .innerJoin(blogPostTranslations, eq(blogPostTranslations.postId, blogPosts.id))
      .where(and(eq(blogPosts.status, 'published'), eq(blogPostTranslations.locale, locale), eq(blogPostTranslations.slug, slug)));
    return row || null;
  },
  ...
  async getAdjacent(locale: BlogLocale, publishedAt: Date) {
    const [prev] = await db.select({
      ...
      .innerJoin(blogPostTranslations, eq(blogPostTranslations.postId, blogPosts.id))
      .where(and(eq(blogPosts.status, 'published'), eq(blogPostTranslations.locale, locale), lt(blogPosts.publishedAt, publishedAt)))
      .orderBy(desc(blogPosts.publishedAt))
      .limit(1);
    ...
    const [next] = await db.select({
      ...
      .innerJoin(blogPostTranslations, eq(blogPostTranslations.postId, blogPosts.id))
      .where(and(eq(blogPosts.status, 'published'), eq(blogPostTranslations.locale, locale), gt(blogPosts.publishedAt, publishedAt)))
...
export type BlogPostDetail = NonNullable<Awaited<ReturnType<typeof blogService.getPublishedBySlug>>>;
