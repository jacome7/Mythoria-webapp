...
describe('creditService.deductCredits', () => {
  it('throws when user cannot afford', async () => {
    ...
    await expect(creditService.deductCredits('user', 10, 'eBookGeneration')).rejects.toThrow(
  ...
  it('records deduction when user can afford', async () => {
    ...
    const addSpy = jest
      ...
      .mockResolvedValue({} as unknown as Awaited<ReturnType<typeof creditService.addCreditEntry>>);

    await creditService.deductCredits('user', 10, 'eBookGeneration', 'story1');

    expect(addSpy).toHaveBeenCalledWith('user', -10, 'eBookGeneration', 'story1');
  });
});
...
describe('creditService.addCredits', () => {
  it('delegates to addCreditEntry', async () => {
    ...
    expect(addSpy).toHaveBeenCalledWith('user', 20, 'creditPurchase', undefined, 'p1');
  });
});
