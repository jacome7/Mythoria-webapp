...
export function normalizeTargetAudience(audience: string | undefined): TargetAudience {
  ...
  const directMatch = getAllTargetAudiences().find(value => 
    value.toLowerCase() === trimmed
  );
  ...
  if (trimmed.includes('baby') || trimmed.includes('infant') || trimmed.includes('0') || trimmed.includes('1') || trimmed.includes('2')) {
    return TargetAudience.CHILDREN_0_2;
  }
  if (trimmed.includes('preschool') || trimmed.includes('3') || trimmed.includes('4') || trimmed.includes('5') || trimmed.includes('6')) {
    return TargetAudience.CHILDREN_3_6;
  }
  if (trimmed.includes('elementary') || trimmed.includes('7') || trimmed.includes('8') || trimmed.includes('9') || trimmed.includes('10')) {
    return TargetAudience.CHILDREN_7_10;
  }
  if (trimmed.includes('middle') || trimmed.includes('tween') || trimmed.includes('11') || trimmed.includes('12') || trimmed.includes('13') || trimmed.includes('14')) {
    return TargetAudience.CHILDREN_11_14;
  }
  if (trimmed.includes('teen') || trimmed.includes('young adult') || trimmed.includes('15') || trimmed.includes('16') || trimmed.includes('17')) {
...
export function normalizeNovelStyle(style: string | undefined): NovelStyle {
  ...
  // Direct match (case-insensitive)
  const directMatch = getAllNovelStyles().find(value => 
    value.toLowerCase() === trimmed
  );
  if (directMatch) return directMatch;
  ...
  if (trimmed.includes('magic') || trimmed.includes('fantasy') || trimmed.includes('dragon') || trimmed.includes('wizard')) {
    return NovelStyle.FANTASY;
  }
  if (trimmed.includes('space') || trimmed.includes('sci') || trimmed.includes('future') || trimmed.includes('robot') || trimmed.includes('alien')) {
    return NovelStyle.SCIENCE_FICTION;
  }
  if (trimmed.includes('scary') || trimmed.includes('horror') || trimmed.includes('frightening') || trimmed.includes('spooky')) {
...
export function normalizeGraphicalStyle(style: string | undefined): GraphicalStyle {
  ...
  // Direct match (case-insensitive)
  const directMatch = getAllGraphicalStyles().find(value => 
    value.toLowerCase() === trimmed
  );
  if (directMatch) return directMatch;
